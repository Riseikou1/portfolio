...
import HandTrackMark from "../components/hand-track-mark";
...
const projects = [
  ...
  { number: "04", title: "Hand-tracking canvas", kind: "Live sites", meta: "COMPUTER VISION / INTERACTION", description: "A real-time finger-tracking experiment that turns hand motion into on-screen drawing with low-latency landmark detection.", href: "https://hand-tracking-canvas.vercel.app/", label: "Explore Hand-tracking canvas", tags: ["Computer vision", "OpenCV", "Interaction"], visual: "hand" },
];
const certificates = [
  ...
  return <main>
    ...
    <section className="hero wrap" id="top">
      <div className="hero-copy">
        ...
        <div className="hero-actions"><a className="button button-dark" href="#work">Explore my work <span>↘</span></a><a className="text-link" href="/contact">Get in touch <span>↗</span></a></div>
      ...
      <div className="hero-visual" aria-label="Temuujin Gerelt-Och working on a laptop">
        ...
        <div className="visual-label bottom-label"><span className="asterisk"><SparkleMark /></span> BUILDING WHAT&apos;S NEXT</div>
    ...
    <section className="work-section wrap" aria-labelledby="work-title"><div className="section-head"><div><p className="section-kicker">THE PORTFOLIO / 01—04</p><h2 id="work-title">Selected projects<span className="period">.</span></h2></div><p>From evidence-led research tools to learning how language models work under the hood.</p></div>
      <div className="filters" role="group" aria-label="Filter projects">{(["All", "Live sites", "Experiments"] as Category[]).map(item => <button key={item} type="button" className={category === item ? "filter active" : "filter"} aria-pressed={category === item} onClick={() => setCategory(item)}>{item} <span>{String(item === "All" ? projects.length : projects.filter(project => project.kind === item).length).padStart(2, "0")}</span></button>)}</div>
      <div className="project-grid">{visibleProjects.map(project => <article className="project-card" key={project.number}>
        <div className={`project-art ${project.visual}`} aria-hidden="true">{project.visual === "research" ? <div className="research-ui"><div className="ui-top"><span>◉ &nbsp; Research GAP</span><span>⌕ &nbsp; ●</span></div><div className="research-question">What does the evidence <em>actually</em> say?</div><div className="research-lines"><span /><span /><span /></div><div className="research-pill"><SparkleMark /> EVIDENCE IN CONTEXT</div></div> : project.visual === "jarvis" ? <div className="jarvis-ui"><span className="jarvis-orb"><SparkleMark /></span><span className="jarvis-name">Jarvisn&apos;t<span className="blink">_</span></span><span className="jarvis-sub">A small model, learning in public.</span><span className="jarvis-input">What can I help with? <b>↑</b></span></div> : project.visual === "price" ? <div className="price-ui"><span className="chart-label">PREDICTED VALUE / MARKET PRICE</span><div className="bar-chart"><i/><i/><i/><i/><i/><i/><i/></div><span className="deal-tag">↓ UNDERVALUED</span></div> : <div className="hand-ui"><span className="hand-cursor"><HandTrackMark /></span><div className="hand-trail" /><span>YOUR HAND. YOUR CANVAS.</span></div>}</div>
    ...
    <section className="about-section" id="about"><div className="wrap about-grid"><div className="about-title"><p className="section-kicker">02 / A LITTLE ABOUT ME</p><h2>Grounded in<br /><em>learning.</em><br />Driven to build<span className="period">.</span></h2></div><div className="about-content"><p className="about-lead">I&apos;m a student at Kookmin University in Seoul, exploring how intelligent systems can make research and everyday work clearer and more useful.</p><p>My interests span machine learning, deep learning, computer vision, and language models. I like understanding the whole pipeline — from data and model training to the experience people actually use.</p><div className="about-facts"><div><span>BASED IN</span><strong>Seoul, South Korea</strong></div><div><span>STUDYING AT</span><strong>Kookmin University</strong></div><div><span>FOCUS AREAS</span><strong>AI · Data Science · Research</strong></div></div><a className="button button-outline" href="/resume.pdf" target="_blank" rel="noreferrer">View my résumé <span>↗</span></a></div></div></section>
    <section className="skills-section wrap" aria-labelledby="skills-title">
      ...
      <div className="skill-grid">
        <div><span>01</span><h3>Build</h3><p>Python, JavaScript, Java, C++, Bash, MySQL, Git/GitHub</p></div>
        <div><span>02</span><h3>Model</h3><p>PyTorch, scikit-learn, neural networks, transformers, cross-attention</p></div>
        <div><span>03</span><h3>Explore</h3><p>Data processing, research synthesis, computer vision, LLMs</p></div>
    ...
    <section className="closing-section"><div className="wrap closing-inner"><div><p className="section-kicker">NEXT / LET&apos;S CONNECT</p><h2>Have an idea<br />worth <em>exploring?</em></h2></div><a href="/contact" className="closing-button">Start a conversation <span>↗</span></a></div></section>
